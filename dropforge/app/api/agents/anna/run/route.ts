...
export async function POST(req: NextRequest) {
  ...
  let body: { cycleNumber?: number; context?: string } = {};
  try { body = await req.json(); } catch { /* OK */ }
  try {
    const report = await runDataBrief(body);
    ...
    const stored = addReport({
      ...
      type: "Growth",
      title: titleMatch?.[1].trim() ?? "Data Brief",
      agents: ["anna"],
      tldr: tldrMatch?.[1].trim().slice(0, 280) ?? "Data Brief",
      tag: "note",
    ...
    return NextResponse.json({ error: err instanceof Error ? err.message : "Anna error" }, { status: 500 });
  }
}
