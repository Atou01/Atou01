...
export async function POST(req: NextRequest) {
  ...
  let body: { cycle?: "J" | "S" | "M"; context?: string } = {};
  try { body = await req.json(); } catch { /* OK */ }
  try {
    const report = await runOpsBrief(body);
    ...
    const stored = addReport({
      ...
      type: "Ops",
      title: titleMatch?.[1].trim() ?? "Ops Brief",
      agents: ["marc"],
      tldr: tldrMatch?.[1].trim().slice(0, 280) ?? "Ops Brief",
      tag: "note",
    ...
    return NextResponse.json({ error: err instanceof Error ? err.message : "Marc error" }, { status: 500 });
  }
}
