...
export async function POST(req: NextRequest) {
  ...
  try {
    const report = await runSupportBrief(body);
    ...
    const stored = addReport({
      ...
      type: "Ops",
      title: titleMatch?.[1].trim() ?? "Support Brief",
      agents: ["sofia"],
      tldr: tldrMatch?.[1].trim().slice(0, 280) ?? "Support Brief",
      tag: "note",
    ...
    return NextResponse.json({ error: err instanceof Error ? err.message : "Sofia error" }, { status: 500 });
  }
}
