...
export async function POST(req: NextRequest) {
  ...
  let body: { weekNumber?: number; brandName?: string; niche?: string } = {};
  try { body = await req.json(); } catch { /* OK */ }
  try {
    const report = await runSocialCalendar(body);
    ...
    const stored = addReport({
      ...
      type: "Content",
      title: titleMatch?.[1].trim() ?? "Social Calendar",
      agents: ["zoe"],
      tldr: tldrMatch?.[1].trim().slice(0, 280) ?? "Social Calendar",
      tag: "note",
    ...
    return NextResponse.json({ error: err instanceof Error ? err.message : "Zoé error" }, { status: 500 });
  }
}
